...
fn Vec(comptime T: type) type {
    return struct {
        ...
        pub fn add(self: &const Vec(T), other: &const Vec(T)) Vec(T) {
            return Vec(T).init(self.x + other.x, self.y + other.y, self.z + other.z);
        }

        pub fn sub(self: &const Vec(T), other: &const Vec(T)) Vec(T) {
            return Vec(T).init(self.x - other.x, self.y - other.y, self.z - other.z);
        }

        pub fn mul(self: &const Vec(T), other: &const Vec(T)) Vec(T) {
            return Vec(T).init(self.x * other.x, self.y * other.y, self.z * other.z);
        }

        pub fn scale(self: &const Vec(T), scaling: T) Vec(T) {
            return Vec(T).init(self.x * scaling, self.y * scaling, self.z * scaling);
        }

        pub fn dot(self: &const Vec(T), other: &const Vec(T)) T {
            return self.x * other.x + self.y * other.y + self.z * other.z;
        }

        pub fn cross(self: &const Vec(T), other: &const Vec(T)) Vec(T) {
...
fn Ray(comptime T: type) type {
    return struct {
        ...
        pub fn init(o: &const Vec(T), d: &const Vec(T)) Ray(T) {
            return Ray(T) {
                .o = o,
                .d = d,
            };
...
fn Sphere(comptime T: type) type {
    return struct {
        ...
        pub fn init(rad: T, p: &const Vec(T), e: &const Vec(T), c: &const Vec(T), refl: Refl_t) Sphere(T) {
            return Sphere(T) {
                .rad = rad,
                .p = p,
                .e = e,
        ...
        pub fn intersect(self: &const Sphere(T), r: &const Ray(T)) T {
...
pub fn toInt(x: var) i32 {
    return i32(math.pow(@typeOf(x), clamp(x), 1.0/2.2) * 255.0 + 0.5);
} 

pub fn intersect(comptime T: type, r: &const Ray(T), t: &T, id: &usize) bool {
    const inf = 1.0e20;
    *t = inf;
...
pub fn main() !void {
    ...
    while (y < h) : (y += 1) {
        ...
        while (x < w) : (x += 1) {
            ...
            while (sy < 2) : (sy += 1) {
                var sx: usize = 0;
                while (sx < 2) : ({sx += 1; r = Vec(ft).init(0.0, 0.0, 0.0);}) {
                    var s: usize = 0;
                    while (s < samps) : (s += 1) {
                        ...
                        const exp1 = ((ft(sx) + 0.5 + dx) / 2.0 + ft(x)) / ft(w) - 0.5;
                        const exp2 = ((ft(sy) + 0.5 + dy) / 2.0 + ft(y)) / ft(h) - 0.5;
                        ...
                        r = r.add(rad.scale(1.0 / ft(samps)));
                    }
